...
class DetectionSettingsWindow(ctk.CTkToplevel):
    def __init__(self, master):
        ...
        self.solidity_slider.grid(row=3, column=0, columnspan=3, padx=10, sticky="ew")
...
class ManhwaCropper(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Manhwa Cropper V2 - Smart Filter")
        self.geometry("1200x800")
        ctk.set_appearance_mode("Dark")

        # --- NEW: Detection settings state ---
        self.min_area_perc = 0.1  # Default: ignore anything smaller than 0.1% of image area
        self.min_solidity = 0.85 # Default: shape must be at least 85% solid
        self.max_aspect_ratio = 25 # Default: ignore shapes 25x wider than tall, or vice versa
        ...
        self.load_button = ctk.CTkButton(self.control_frame, text="Load", command=self.load_images)
    ...
    def open_settings(self):
        """Opens the detection settings window."""
    ...
    def run_auto_detect(self):
        ...
        # --- MODIFIED: Multi-step filtering logic ---
        for cnt in contours:
            # 1. Area Filter
            ...
            # If all checks pass, it's a valid panel
    ...
    def save_all_crops(self):
        if not self.selections: self.status_label.configure(text="No regions selected to save."); return
        if not self.output_directory and not self.prompt_for_output_directory(): return
        sc = 0
        for selection in self.selections:
            coords = selection['coords']
            x1 = (min(coords[0], coords[2]) - self.image_x) / self.zoom_level; y1 = (min(coords[1], coords[3]) - self.image_y) / self.zoom_level
            x2 = (max(coords[0], coords[2]) - self.image_x) / self.zoom_level; y2 = (max(coords[1], coords[3]) - self.image_y) / self.zoom_level
            ...
            cropped_image.save(save_path, "PNG"); self.crop_counter += 1; sc += 1
        self.status_label.configure(text=f"Saved {sc} panels!"); self.clear_selections(update_status=False, save_state=True)
